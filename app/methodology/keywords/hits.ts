...
export async function buildKeywordHits(
    cats: { items: string[] }[],
    dailyDirRel: string = path.join("content", "daily")
): Promise<HitsIndex> {
    const dailyDir = path.join(process.cwd(), dailyDirRel);
    const files = await fs.readdir(dailyDir).catch(() => []);

    const allKeywords = cats.flatMap((c) => c.items);
